...
anomaly_detector = AnomalyDetector(
			model_file=Path(__file__).parents[2] / "src/anomaly_detection/model_checkpoints/cookie/model.pt",
...
while True:
	...
	if frame is not None:
		detection_result = anomaly_detector_processor.process_frame(frame)
		detection_result = asdict(detection_result)
		detection_result = {key: bytes_to_cv2_image(value) for key, value in detection_result.items()}
