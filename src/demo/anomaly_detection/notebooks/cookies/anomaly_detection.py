...
def _():
    ...
    df_history[df_history["epoch"] > 5].plot(
        x="epoch", y=["train_reconst_mse", "test_reconst_mse"]
    )
    return
...
@app.cell
def _():
    quantile = mo.ui.slider(
        start=0, stop=1, step=0.01, value=0.9, label="Quantile", show_value=True
    )
    quantile
    return (quantile,)
...
def _(df, quantile):
    ...
    print(
        classification_report(
            df_clf["label"], df_clf["prediction"], target_names=["good", "damaged"]
        )
    )
