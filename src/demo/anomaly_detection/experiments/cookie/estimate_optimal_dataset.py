...
OUTPUT_DIR: Path = Path(os.environ["COOKIE_OUTPUT_DIR"])
DATASET_DIR: Path = Path(os.environ["COOKIE_DATASET_DIR"])
BASE_AUTO_ENCODER: Path = Path(os.environ["COOKIE_CKPT_DIR"]).joinpath("ae_model.pt")
...
def main() -> None:
    ...
    for ds_size in range(20, 200, 20):
        load_model(model, BASE_AUTO_ENCODER)

        ds_train, ds_val = _load_trainig_subset(ds, ds_size)

        history = _run_training(
            model=model, ds_train=ds_train, ds_val=ds_val, epochs=100
        )

        model.to("cpu").eval()
        model.determine_decision_boundary(calibration_data=ds_val[:][0], quantile=0.95)

        report_dir = OUTPUT_DIR.joinpath(str(ds_size))
        report_dir.mkdir(parents=True, exist_ok=True)
        # _write_classification_report(
        #     autoencoder=model,
        #     ds=_load_clf_dataset(),
        #     report_file=report_dir.joinpath("classification_report.txt"),
        # )
        defs.save_model(model, report_dir.joinpath("ae_model.pt"))
        defs.save_history(history, report_dir.joinpath("ae_history.csv"))
...
def _load_trainig_subset(
    base_ds: CookieAdDataset, ds_size: int
) -> tuple[Dataset, Dataset]:
    indices_to_use = [randint(0, len(base_ds) - 1) for _ in range(ds_size)]
    ds_subset = Subset(base_ds, indices_to_use)
    ds_train, ds_val = random_split(ds_subset, lengths=[0.8, 0.2])
